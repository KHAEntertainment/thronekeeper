...
The CLI is fully functional with the shared `lib/` modules. The VS Code extension (`extensions/claude-throne/`) currently has its own implementations of:
...
Move `lib/` to a separate npm package (`@thronekeeper/core`)
