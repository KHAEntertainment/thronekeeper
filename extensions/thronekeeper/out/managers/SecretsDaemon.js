...
exports.SecretsDaemonManager = void 0;
...
class SecretsDaemonManager {
    ...
    async start(context, pythonPath) {
        ...
        this.proc = (0, child_process_1.spawn)(python, [
            '-m', 'ct_secretsd',
            '--host', host,
            '--port', String(port),
            '--auth-token', token,
    ...
    authHeaders() {
        if (!this.info)
            throw new Error('secrets daemon not running');
        return {
    ...
    async saveProviderKey(providerId, apiKey) {
        if (!this.info)
            throw new Error('secrets daemon not running');
    ...
    async stopProxy() {
        ...
        if (!res.ok)
            throw new Error(`stop proxy failed: ${res.status}`);
        const data = await res.json();
        return Boolean(data?.success);
    }
    async waitForHealth(url, timeoutMs = 10000) {
    ...
    async resolveBackendPath(context) {
        ...
        const configuredBackend = vscode.workspace.getConfiguration('claudeThrone').get('backendPath')?.trim();
        ...
        if (workspaceFolders) {
            for (const folder of workspaceFolders) {
                ...
                if (workspaceName.includes('claude') || workspaceName.includes('throne')) {
                    ...
                    searchPaths.push(path.join(parentDir, 'thronekeeper'), path.join(parentDir, 'claude-throne'), path.join(parentDir, 'mighty-morphin-claude'), path.join(parentDir, 'claude-throne-main'));
                }
    ...
    async resolvePython(backendRoot, pythonPath) {
        // Check configured path first
        const configured = pythonPath || vscode.workspace.getConfiguration('claudeThrone').get('pythonInterpreterPath');
        if (configured) {
...
exports.SecretsDaemonManager = SecretsDaemonManager;
//# sourceMappingURL=SecretsDaemon.js.map