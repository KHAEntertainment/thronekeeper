...
class PanelViewProvider {
     ...
     */
    getConfigurationTarget(applyScope = 'workspace') {
     ...
     */
    normalizeProviderMap(providerModels, providerId) {
        ...
        const unexpectedKeys = Object.keys(providerModels).filter(key => !allowedKeys.includes(key) && providerModels[key] !== undefined && providerModels[key] !== null);
     ...
     */
    getCodingModelFromProvider(providerModels, providerId) {
     ...
     */
    async hydrateGlobalKeysFromProvider(providerId, reasoningModel, completionModel, valueModel, twoModelMode) {
    ...
    post(message) {
        ...
        const validated = (0, messages_1.safeValidateMessage)(message, 'toWebview', (msg) => {
    ...
    get configProvider() {
        const cfg = vscode.workspace.getConfiguration('claudeThrone');
        return cfg.get('provider', 'openrouter');
    }
    constructor(ctx, secrets, proxy, log) {
        this.ctx = ctx;
        this.secrets = secrets;
        this.proxy = proxy;
        this.log = log;
        this.currentProvider = 'openrouter'; // Runtime source of truth for current provider selection
        ...
        const selectedCustomProviderId = cfg.get('selectedCustomProviderId', '');
    ...
    resolveWebviewView(webviewView) {
        ...
        webviewView.webview.onDidReceiveMessage(async (msg) => {
            ...
            if (enableValidation) {
                ...
                const normalized = (0, messages_1.normalizeMessageType)(msg);
                const validated = (0, messages_1.safeValidateMessage)(normalized, 'toExtension', (validationMsg) => {
    ...
    async postStatus() {
        ...
        if (!valueModel)
            valueModel = String(cfg.get('valueModel') || '');
        this.post({
            type: 'status',
            payload: {
                ...s,
    ...
    postConfig() {
        ...
        const opusPlanMode = config.get('opusPlanMode', false);
        ...
        const cachedTimestamp = config.get('anthropicDefaultsTimestamp', 0);
        const cachedDefaults = config.get('anthropicDefaults', null);
        let cacheAgeDays = 0;
    ...
    async handleListModels(freeOnly, requestToken, requestedProvider) {
        ...
        const cfg = vscode.workspace.getConfiguration('claudeThrone');
        const debugMode = cfg.get('proxy.debug', false);
        // Removed bypass for Deepseek/GLM to allow dynamic model fetching
        let manualCustomBaseUrl;
        if (provider === 'custom') {
            manualCustomBaseUrl = cfg.get('customBaseUrl', '');
        ...
        try {
            ...
            const modelIds = await (0, Models_1.listModels)(provider, baseUrl, apiKey);
            ...
            if (errorType === 'generic') {
                ...
                else if (errorStr.includes('timed out') || errorStr.includes('timeout') || err.name === 'AbortError') {
                    errorType = 'timeout';
    ...
    async postCustomProviders() {
        try {
            ...
            const endpointOverrides = cfg.get('customEndpointOverrides', {});
            this.post({
    ...
    async postPopularModels() {
        ...
        try {
            ...
            const savedCombosByProvider = cfg.get('savedCombosByProvider', {});
    ...
    async handleUpdateProvider(provider) {
        ...
        try {
            ...
            const customProviders = cfg.get('customProviders', []);
            const customProvider = customProviders.find(p => p.id === provider);
            const target = this.getConfigurationTarget(cfg.get('applyScope', 'workspace'));
            if (customProvider) {
    ...
    async handleUpdateCustomUrl(url) {
        const cfg = vscode.workspace.getConfiguration('claudeThrone');
        const target = this.getConfigurationTarget(cfg.get('applyScope', 'workspace'));
    ...
    async handleSaveCombo(name, reasoningModel, codingModel, valueModel, providerId) {
        try {
            ...
            const config = vscode.workspace.getConfiguration('claudeThrone');
            const savedCombosByProvider = config.get('savedCombosByProvider', {});
            const providerCombos = savedCombosByProvider[this.currentProvider] || [];
    ...
    async handleSaveCustomProvider(name, baseUrl, id) {
        try {
            // Validate inputs
            if (!name || !name.trim()) {
            ...
            const target = this.getConfigurationTarget(config.get('applyScope', 'workspace'));
    ...
    async handleDeleteCustomProvider(id) {
        try {
            if (!id || !id.trim()) {
    ...
    // Comment 3: Handle endpoint kind update for custom provider
    async handleUpdateEndpointKind(baseUrl, endpointKind) {
        try {
            const normalizedUrl = baseUrl.replace(/\/+$/, '');
            const cfg = vscode.workspace.getConfiguration('claudeThrone');
            const currentOverrides = cfg.get('customEndpointOverrides', {});
    ...
    async handleStoreKey(provider, key) {
    ...
    async handleStoreAnthropicKey(key) {
        try {
            if (!key || !key.trim()) {
            ...
            try {
                ...
                if (refreshErr.message?.includes('unsaved changes') || refreshErr.message?.includes('CodeExpectedError')) {
                    vscode.window.showInformationMessage('Anthropic API key stored. To update model defaults, please save your VS Code settings and run "Thronekeeper: Refresh Anthropic Defaults".');
                }
                else {
                    vscode.window.showWarningMessage(`Anthropic API key stored, but could not refresh defaults: ${refreshErr.message}`);
                }
            }
    ...
    async handleStartProxy() {
        try {
            ...
            const hydrationSuccess = await this.hydrateGlobalKeysFromProvider(this.runtimeProvider, reasoningModel, completionModel, valueModel, twoModelMode);
            if (!hydrationSuccess) {
    ...
    async handleRevertApply() {
        try {
            ...
            if (err?.message?.includes('not a registered configuration') || err?.name === 'CodeExpectedError') {
                const action = await vscode.window.showWarningMessage('Configuration error during revert. Some settings may not be available. Try checking configuration health.', 'Check Config Health', 'Reload VS Code');
    ...
    async handleSaveModels(data) {
        try {
            ...
            try {
                const verification = cfg.get('modelSelectionsByProvider', {});
    ...
    async handleFilterModels(filter) {
    ...
    async handleToggleTwoModelMode(enabled) {
        ...
        if (!enabled) {
            const currentOpusPlanMode = cfg.get('opusPlanMode', false);
    ...
    async handleOpusPlanToggle(enabled) {
    ...
    async handleUpdateDebug(enabled) {
    ...
    getHtml() {
        ...
        const jsUri = this.view.webview.asWebviewUri(vscode.Uri.joinPath(this.ctx.extensionUri, 'webview', 'main.js'));
        return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
